...
function Toast({ msg, onDone }: { msg: string; onDone: () => void }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ width: 20, height: 20, borderRadius: 99, background: "#16a34a", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
...
export default function TopicDiscovery({ onNavigate, onAddToQueue }: TopicDiscoveryProps) {
  ...
  const P = isMobile ? "20px 16px" : "28px 32px";
  ...
  const handleDiscover = () => {
    ...
    const interval = setInterval(() => {
      ...
      if (p >= 100) {
        ...
        setTimeout(() => {
          setResults(MOCK_DISCOVERED.slice(0, parseInt(count)));
          setIsRunning(false);
          setHasRun(true);
          setProgress(100);
        }, 400);
  ...
  return (
    <div className={s.wrapper} style={{ padding: P }}>
      ...
      <div className={s.pageHeader} style={{ marginBottom: isMobile ? 18 : 24 }}>
        <h1 className={s.pageTitle} style={{ fontSize: isMobile ? 18 : 22 }}>
          Gợi ý chủ đề
        </h1>
        <p className={s.pageSubtitle}>
          Hệ thống đề xuất các chủ đề có tiềm năng nội dung cao.
        </p>
      </div>
      ...
      <div className={s.configPanel}>
        ...
        <div style={{
          display: "grid",
          gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr 120px 120px",
          gap: 12,
          marginBottom: 16,
        }}>
          <div>
        ...
        <button
          ...
          disabled={isRunning}
          style={{
            padding: "9px 20px", background: isRunning ? "#93c5fd" : "#2563eb",
        ...
        >
          {isRunning ? (
            <>
              <span style={{ display: "inline-block", width: 12, height: 12, border: "2px solid rgba(255,255,255,0.4)", borderTopColor: "#fff", borderRadius: 99, animation: "spin 0.7s linear infinite" }} />
              Đang gợi ý...
            </>
          ) : (
            <>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", background: "#f8fafc", border: "1px solid #e5e7eb", borderRadius: 8, marginBottom: 12, flexWrap: "wrap" }}>
            ...
            <span style={{ fontSize: 11, color: "#6b7280", lineHeight: 1.5 }}>
              Điểm ở đây là <strong style={{ color: "#374151" }}>dự đoán</strong> từ xu hướng tìm kiếm — chưa phải kết quả nghiên cứu thực sự.
              Thêm vào hàng đợi → nghiên cứu → kết quả hiện ở <strong style={{ color: "#374151" }}>Ưu tiên viết</strong>.
            </span>
          </div>
                    ...
                    <div style={{ minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5, flexWrap: "wrap" }}>
                        ...
                        <span style={{
                          ...
                          background: topic.priority === "high" ? "#eff6ff" : topic.priority === "medium" ? "#fffbeb" : "#f9fafb",
                          color: topic.priority === "high" ? "#2563eb" : topic.priority === "medium" ? "#d97706" : "#9ca3af",
                        }}>
                          {topic.priority === "high" ? "Ưu tiên cao" : topic.priority === "medium" ? "Ưu tiên trung bình" : "Ưu tiên thấp"}
                        </span>
                      </div>
      ...
      <div className={s.autoPanel}>
        <div className={s.autoPanelHeader} style={{ marginBottom: autoEnabled ? 16 : 0 }}>
          <div>
            ...
            <p className={s.autoPanelSubtitle}>
              AI tự chạy định kỳ và đề xuất chủ đề mới vào dashboard.
            </p>
          </div>
          {/* Toggle */}
          <button
            ...
            <div style={{ display: "flex", alignItems: "flex-end" }}>
              <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
                ...
                <span style={{ fontSize: 12, color: "#374151", lineHeight: 1.4 }}>
                  Tự động thêm chủ đề điểm cao vào hàng đợi
                </span>
              </label>
            </div>
          </div>
      ...
      {toastMsg && (
        <Toast key={toastKey.current} msg={toastMsg} onDone={() => setToastMsg(null)} />
      )}
    </div>
  );
}
